...
// TODO: handle this in a consistent and simpler way after AMD MFMA support has been added
    ...
    if (!(std::is_same<T, half2>::value || std::is_same<T, nv_bfloat162>::value) || rows_per_block != MMF_ROWS_PER_BLOCK) {NO_DEVICE_CODE;} else {
    ...
    if (rows_per_block != MMF_ROWS_PER_BLOCK_CDNA) {NO_DEVICE_CODE;} else {
    typedef tile<16, 8,  T,     DATA_LAYOUT_I_MAJOR> tile_A;
    typedef tile<16, 8,  T,     DATA_LAYOUT_I_MAJOR> tile_B;
    typedef tile<16, 16, float, DATA_LAYOUT_J_MAJOR> tile_C;
#else
#ifdef VOLTA_MMA_AVAILABLE
    if (!std::is_same<T, half2>::value || rows_per_block != MMF_ROWS_PER_BLOCK) {NO_DEVICE_CODE;} else {
...
#else
    if (rows_per_block != MMF_ROWS_PER_BLOCK) {NO_DEVICE_CODE;} else {
...
#pragma unroll
            ...
            } else if (std::is_same<T, half2>::value || std::is_same<T, nv_bfloat162>::value) {
#pragma unroll
                for (int j0 = 0; j0 < tile_B::I; ++j0) {
                    const int j = j0 + itB*tile_B::I;
